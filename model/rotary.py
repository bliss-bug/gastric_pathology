...
class Rotary2D:
    ...
    def forward(self, x_shape):
        ...
        if self.pos_cached is None or self.w_size_cached != W or self.h_size_cached != H:
            ...
            self.pos_cached = torch.empty(H * W, self.dim, dtype=torch.float).to(x_shape.device)
            for i in range(H):
                for j in range(W):
                    emb = torch.cat([
                        position_x[i, 0::2],
                    ...
                    self.pos_cached[i * W + j] = emb.to(torch.float).to(x_shape.device)
        return self.pos_cached

if __name__ == "__main__":
    rotary = Rotary2D(dim=384)
    data = torch.tensor([300,300]).cuda()
